...
%% ------------------------------------------------------------------------
\maketitle
<<echo=FALSE,eval=TRUE,results=tex,prefix.string=example-GMD,just="center">>=
...
\texttt{hello-GMD.R} (fig. \ref{src:hello-GMD.R}) is a minimal example to load and check of that your \GMD{} installation works. It also includes code for viewing the package information and this ``Vignette'', checking data sets provided by \GMD{}, starting a demo and listing the citation of \GMD{}.
...
\texttt{example-ghist.R} (fig. \ref{src:example-ghist.R}) is an example on how to construct a histogram object from raw data and make a visualization based on this.
...
\texttt{case-nottem.R} (fig. \ref{src:case-nottem.R}) is a study on how to draw histograms side-by-side and to compute and visualize a bin-wise summary plot, using air temperature data at Nottingham Castle.
...
\label{md:gdist}
\texttt{example-gdist.R} (fig. \ref{src:example-gdist.R}) is an example on how to measure distances using a user-defined metric, such as \textit{correlation distance} and \textit{GMD}.
